...
The fixed MPID direct-detector comparison has now been run. Job 938745 achieved internal MPID filename-family mAP50 0.761 after two epochs, but UK transfer exposed partial-assembly boxes, missed ceramic strings, glass/composite confusion and a high-score ceramic-to-composite error. On the one image-level prospective porcelain check, the complete-string match was wrong and rejected to `unknown`; a shorter wrong-material fragment still passed the per-box gate. The direct detector is therefore retained only as a proposal arm. See `MPID_MATERIAL_DETECTOR_V1.md`.
...
The UK pool is allowed to grow. `acquire_uk_material_sources.py` starts a provenance-preserved candidate pool with nine explicit Geograph material pages. Two were already consumed by earlier adaptive development and cannot become new evaluation images; two Mayles Lane photos share one asset/work group and must stay in the same split; one old telegraph-pole record is auxiliary rather than primary electricity-network evidence. The Boyatt Wood glass transmission image and Chilcomb ceramic distribution image add two independent source groups. A second Boyatt Wood photograph supplies a prospective porcelain check but stays in the already-observed Boyatt Wood asset group. The remaining records are still candidates until their original pixels and compatible whole-assembly boxes pass review. Every record retains the page URL, original image URL, author, licence, page/image hashes, short evidence excerpt and asset group.

`build_uk_source_pool.py` combines these records with the existing 27-image UK qualitative pool without copying or relabelling pixels. The resulting 36-record manifest explicitly separates provenance-only images from source-evidenced material candidates. It is an expandable pool, not a frozen test set.

The public pixel decision record is `UK_MATERIAL_SOURCE_PIXEL_AUDIT_V1.json`. Acquired originals and the generated 36-record manifest stay in the ignored data cache; the acquisition and manifest builders are source-released.
