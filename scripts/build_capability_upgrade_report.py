...
LOCALISATION_ADAPTATION_V1_RESULT_SHA = "4e25c9f42b247b0c93e3308a83a8a37584bbf5bae9b986baf1811909049cf948"
LOCALISATION_ADAPTATION_V2_RESULT_SHA = "af25826dac56a24e57eda3713b5613c0df7eead7834cbd76b46934eaca55790b"
...
def load_localisation_adaptation_result(version, expected_sha):
    path = ROOT / f"runs/uk_insulator_adaptation/{version}_20260830/results.json"
...
<section class="panel warning"><strong>This is not yet the supplied Keen AI result.</strong><p>A second fixed target-domain adaptation moved the untouched UK v3 cohort from __ADAPT_BASE_TP__/30 true positives for the frozen v1 checkpoint to __ADAPT_TP__/30, with __ADAPT_FP__ false positives. It still missed __ADAPT_FN__ objects and produced __ADAPT_NEG_FP__ false detections on two source-evidenced telecom/telephone negatives. TTPLA steel structure is a transmission-tower assembly task, and the pole-top benchmark is a publisher-mask-derived shaft endpoint rather than a physical component detector.</p><a href="../index.html">← UK review workbench</a> · <a href="../material_prospective/index.html">UK material audit</a> · <a href="../localisation_adaptation_v2/index.html">UK adaptation v2 audit</a> · <a href="../localisation_adaptation/index.html">v1 audit</a> · <a href="http://127.0.0.1:8771/report/index.html">EPRI component explorer ↗</a></section>
<section class="cards"><div class="card"><small>UK localisation adaptation v2 · prospective</small><strong>__ADAPT_TP__ / 30</strong><p>The fixed v2 specialist produced __ADAPT_FP__ false positives and missed __ADAPT_FN__ references; the frozen v1 checkpoint recovered __ADAPT_BASE_TP__/30. <a href="../localisation_adaptation_v2/index.html">Inspect all nine assets</a>.</p></div><div class="card"><small>UK material · prospective oracle regions</small><strong>__MAT_CORRECT__ / __MAT_ACCEPTED__</strong><p>__MAT_ACC__ accepted accuracy at __MAT_COVERAGE__ coverage across __MAT_TOTAL__ source-assisted regions. Full-image localisation matched __LOC_MATCHED__/__MAT_TOTAL__. <a href="../material_prospective/index.html">Inspect all five assets</a>.</p></div><div class="card"><small>Lattice steel structure · fixed test</small><strong>__STEEL_TP__ / __STEEL_TOTAL__</strong><p>Supervised matched instances; open vocabulary matched __OPEN_TP__/__STEEL_TOTAL__. Both had zero detections on __STEEL_NEG__ hard-negative images at score 0.25.</p></div><div class="card"><small>Pole-shaft endpoint · development</small><strong>__POLE_ACCEPTED__ / __POLE_ELIGIBLE__</strong><p>Mask geometry coverage versus __BOX_ACCEPTED__/__POLE_ELIGIBLE__ for box geometry. Accepted mask outputs had median normalised error __POLE_MEDIAN__.</p></div></section>
...
<script id="payload" type="application/json">""" + payload + """</script><script>'use strict';const D=JSON.parse(document.getElementById('payload').textContent);function setup(prefix,rows,caption){let i=0,s=document.getElementById(prefix+'Select'),img=document.getElementById(prefix+'Image'),count=document.getElementById(prefix+'Counter'),cap=document.getElementById(prefix+'Caption');rows.forEach((r,j)=>{let o=document.createElement('option');o.value=j;o.textContent=r.image_id+(r.file_name?' · '+r.file_name:'');s.append(o)});function show(n){i=(n+rows.length)%rows.length;let r=rows[i];s.value=i;img.src=r.image;count.textContent=(i+1)+' / '+rows.length;cap.textContent=caption(r)}document.getElementById(prefix+'Prev').onclick=()=>show(i-1);document.getElementById(prefix+'Next').onclick=()=>show(i+1);s.onchange=()=>show(Number(s.value));show(0)}setup('steel',D.steelwork.gallery,r=>r.selection_kind+' · publisher instances '+r.reference_count+' · open TP/FP/FN '+r.open_vocabulary.tp+'/'+r.open_vocabulary.fp+'/'+r.open_vocabulary.fn+' · supervised '+r.supervised.tp+'/'+r.supervised.fp+'/'+r.supervised.fn);setup('pole',D.pole_top.gallery,r=>'mask '+r.mask_model.status+(r.mask_model.normalized_error!==undefined?' · normalised error '+r.mask_model.normalized_error.toFixed(3):'')+' · box '+r.box_model.status+(r.box_model.normalized_error!==undefined?' · normalised error '+r.box_model.normalized_error.toFixed(3):''));</script></body></html>"""
    ...
    localisation_adaptation = data["localisation_adaptation_v2"]
    ...
    return (page.replace("__MAT_ACC__", f'{material["accepted_accuracy"]:.1%}')
                ...
                .replace("__ADAPT_BASE_TP__", str(localisation_adaptation["baseline_v1_adapted_full_plus_tiles"]["tp"]))
                ...
                .replace("__ADAPT_NEG_FP__", str(sum(row["fp"] for row in localisation_adaptation["adapted_specialist_full_plus_tiles"]["per_image"] if row["role"] == "hard_negative")))
...
def build(report_root):
    ...
    localisation_adaptation_v1 = load_localisation_adaptation_result(
        "v1", LOCALISATION_ADAPTATION_V1_RESULT_SHA)
    localisation_adaptation_v2 = load_localisation_adaptation_result(
        "v2", LOCALISATION_ADAPTATION_V2_RESULT_SHA)
    data = {
        ...
        "localisation_adaptation_v2": localisation_adaptation_v2,
        "raw_results": {"steelwork_sha256": STEEL_RESULT_SHA, "pole_top_sha256": POLE_RESULT_SHA,
                        ...
                        "uk_localisation_adaptation_v1_sha256": LOCALISATION_ADAPTATION_V1_RESULT_SHA,
                        "uk_localisation_adaptation_v2_sha256": LOCALISATION_ADAPTATION_V2_RESULT_SHA},
    }
