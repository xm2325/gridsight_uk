...
def main():
    record=ROOT/'runtime/material_head_submission.json'
    cfg_path=ROOT/'configs/material_head_v1.json';cfg=load(cfg_path)
    manifest=ROOT/cfg['dataset']/'manifest.json'
    assert load(manifest)['protocol_sha256']==sha(cfg_path)
    assert sha(ROOT/cfg['encoder']/'model.safetensors')==cfg['encoder_sha256']
    ...
    queue=subprocess.check_output(['squeue','--noheader','--name=gridsight-material-head-v1','--format=%i'],text=True).strip()
    if queue:
    ...
    receipt={'status':'SUBMITTING','created_at':datetime.now(timezone.utc).isoformat(),
             ...
             'runner_sha256':sha(ROOT/'scripts/roihu_material_head.py'),
             'sbatch_sha256':sha(ROOT/'scripts/material_head.sbatch')}
    record.parent.mkdir(parents=True,exist_ok=True)
    with record.open('x') as f:
        json.dump(receipt,f,indent=2)
    result=subprocess.run(['sbatch','--parsable','scripts/material_head.sbatch',receipt['manifest_sha256']],cwd=ROOT,capture_output=True,text=True)
