#!/usr/bin/env python3
"""Bounded UK insulator specialist adaptation and prospective evaluation."""
...
def main(config_name):
    ...
    if not acceptance_manifest["selection_frozen_before_adapted_model_inference"] or acceptance_manifest["model_inference_performed_before_freeze"]:
    ...
    (out / "code").mkdir()
    snapshots = {}
    snapshot_paths = [config_path, Path(__file__), ROOT / "scripts/uk_insulator_adaptation_v1.sbatch",
                      ROOT / "scripts/prepare_uk_insulator_adaptation_v1.py",
                      ROOT / "scripts/acquire_uk_insulator_localisation_v2.py",
    ...
    try:
        ...
        baseline_rows = evaluate_model(baseline, "baseline_epri", cfg["baseline_insulator_class_id"],
                                       records, target, evaluation, torch)
        result["acceptance_predictions"]["baseline_epri"] = baseline_rows
        write(out / "results.json", result)
        del baseline
        gc.collect()
        torch.cuda.empty_cache()
        ...
        result["metrics"] = {
            "baseline_epri_full_plus_tiles": totals_for(out, baseline_rows, records, evaluation["operating_scores"], evaluation["evaluation_ious"]),
        ...
        for name, rows in (("baseline_epri_full", baseline_rows), ("adapted_specialist_full", adapted_rows)):
            copied = []
            for row in rows:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/uk_insulator_adaptation_v1.json")
