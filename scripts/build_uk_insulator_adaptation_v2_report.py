...
RUN = ROOT / "runs/uk_insulator_adaptation/v2_20260830"
SOURCE = ROOT / "data/external/uk_insulator_localisation_v3"
DEFAULT_OUT = ROOT / "runs/uk_capabilities/v3_20260827/report/localisation_adaptation_v2"
RESULT_SHA = "af25826dac56a24e57eda3713b5613c0df7eead7834cbd76b46934eaca55790b"
MANIFEST_SHA = "d74f206e506c9c61303cdf20c092c44c107332cc3931ccf0f6a8079e68ac50ac"
...
def build(output=DEFAULT_OUT):
    ...
    baseline_rows = {row["record_id"]: row for row in result["acceptance_predictions"]["baseline_v1_adapted"]}
    ...
    for record_id, record in records.items():
        ...
        raw_links = {}
        for model_name, label, rows in (("baseline_v1_adapted", "v1 UK-adapted baseline · full + tiles", baseline_rows),
                                         ("adapted_specialist", "v2 UK-adapted specialist · full + tiles", adapted_rows)):
    ...
    data = {"status": result["status"], "job_id": result["runtime"]["slurm_job_id"],
            ...
            "conclusion": "The fixed v2 adaptation improved the primary prospective count from 5/30 to 7/30, but missed 23 references and increased false detections on source-evidenced telephone/telecom negatives. This is not yet Keen-style dense component localisation."}
    write_json(output / "data.json", data)
    cards = "".join(
        f'<div class="metric"><small>{label}</small><b>{primary[name]["tp"]}/30</b><span>recall {primary[name]["recall"]:.1%} · FP {primary[name]["fp"]}</span></div>'
        for name, label in (("baseline_v1_adapted_full_plus_tiles", "v1 adapted · full + tiles"),
                            ("adapted_specialist_full_plus_tiles", "v2 adapted · full + tiles"),
                            ("adapted_specialist_full", "v2 adapted · full frame")))
...
<title>UK insulator adaptation v2 audit</title><style>
...
</style></head><body><main><div>GridSight-UK · prospective adaptation audit</div><h1>UK small-insulator adaptation · v2</h1>
<p>Nine untouched UK assets · 30 analyst visible-object references · two source-evidenced hard negatives. The checkpoint and all settings were frozen before acceptance inference. Primary point: raw score ≥ {score}, IoU ≥ {overlap}.</p>
<div class="warning"><b>Small localisation gain, substantial deployment gap.</b> The v2 specialist recovered 7/30 references with 7 false positives, versus 5/30 with 6 false positives for the frozen v1 checkpoint. It missed 23 objects and produced six false detections across the two hard negatives. Scores are uncalibrated and are not probabilities.</div>
