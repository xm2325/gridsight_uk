...
def make_thumb(src: Path, dst: Path, max_size=(560, 420)) -> tuple[int, int]:
    ...
    with Image.open(src) as im:
        rgb = im.convert("RGB")
        thumb = ImageOps.contain(rgb, max_size, Image.Resampling.LANCZOS)
        thumb.save(dst, "WEBP", quality=90, method=6)
        return thumb.size


def copy_full(src: Path, dst: Path) -> tuple[int, int, int]:
    dst.parent.mkdir(parents=True, exist_ok=True)
...
def card(entry: dict) -> str:
    ...
    <article class="dataset-card" data-filter="{html.escape(entry['filter_group'])}">
      ...
      <div class="dataset-card-body">
        ...
        <div class="dataset-badges">{source_badge}{split_badge}{role_badges}</div>
        <p>{html.escape(entry['description'])}</p>
        <div class="dataset-hash">SHA-256 · {html.escape(entry['sha256'][:16])}…</div>
      </div>
...
def main() -> None:
    ...
    for item in legacy_manifest["images"]:
        ...
        src = ROOT / runtime["path"]
        roles, group, description = role_map[rid]
        full_name = f"{rid}.jpg"
        thumb_name = f"{rid}.webp"
        w, h, nbytes = copy_full(src, FULL / full_name)
        tw, th = make_thumb(src, THUMBS / thumb_name)
...
<section id="dataset">
  ...
  <p class="muted dataset-intro">Every unique source currently represented by the repository manifests is shown below. Roles are intentionally separated: training/validation, frozen holdout, material development, and scale-stress review are not interchangeable evidence.</p>
  <div class="dataset-summary">
    ...
    <div class="metric"><b>3</b><span>v4.4 angle / material / terminal sources</span></div>
  </div>
  ...
  <div class="panel provenance-panel">
    ...
    <div class="provenance-grid">
      ...
      <span>✓ original source page linked</span>
      <span>✓ local runtime image retained for the deployed gallery</span>
      <span>✓ SHA-256 identity recorded</span>
...
(function(){
  ...
  filters.forEach(btn=>btn.addEventListener('click',()=>{
    ...
    groups.forEach(group=>{
      const visible=[...group.querySelectorAll('.dataset-card')].some(x=>!x.hidden);
  ...
  document.querySelectorAll('.dataset-image-button').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    if(modal && image){ image.src=btn.dataset.full; modal.classList.add('open'); modal.setAttribute('aria-hidden','false'); }
...
'''
    ...
    manifest = {
        "version": "v4.8-full-dataset-gallery",
    ...
    print(json.dumps({
        ...
        "full_image_bytes": sum((FULL / f"{rid}.jpg").stat().st_size for rid in ids),
