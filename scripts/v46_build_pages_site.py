...
def main() -> None:
    ...
    for label, key in method_order:
        ...
        cards.append(
            f'<button class="method-card{" selected" if selected else ""}" data-method="{key}">'
...
<section id="detection"><div class="eyebrow">Frozen final holdout</div><h2>A development heuristic failed under new tower morphology</h2><div class="two"><div class="panel"><h3>Recall at IoU ≥ 0.30</h3><div class="barrow"><span>Raw YOLOE text</span><div class="bar"><i style="width:{100*raw30['recall']:.1f}%"></i></div><b>{pct(raw30['recall'])}</b></div><div class="barrow"><span>Geometry filter</span><div class="bar geo"><i style="width:{100*geo30['recall']:.1f}%"></i></div><b>{pct(geo30['recall'])}</b></div></div><div class="panel"><h3>What changed?</h3><p class="muted">Raw YOLOE kept 10 true positives with 2 false negatives. The frozen geometry method kept only 1 true positive and rejected 11 reference insulators. The negative result was preserved instead of tuning against the final holdout.</p></div></div></section>
<section id="shift"><div class="eyebrow">Failure diagnosis</div><h2>Quantified morphology shift</h2><div class="shift"><div class="metric"><b>{shift['train_d2_max']:.3f}</b><span>maximum training Mahalanobis d²</span></div><div class="metric warn"><b>{shift['shifted_median_d2']:.3f}</b><span>median d² on the most shifted final tower</span></div><div class="metric"><b>{shift['train_width_ratio']:.3f}</b><span>training median insulator width / tower width</span></div><div class="metric warn"><b>{shift['shifted_width_ratio']:.3f}</b><span>shifted-tower median width / tower width</span></div></div></section>
...
function render(key){{overlay.innerHTML='';const preds=Object.fromEntries(DATA.methods[key].map(x=>[x.id,x]));for(const [id,box] of Object.entries(DATA.boxes)){{const p=preds[id];if(!p)continue;const [x1,y1,x2,y2]=box;const glass=p.predicted_label==='glass';const c=glass?'#45e39f':'#ffb35d';overlay.appendChild(el('rect',{{x:x1,y:y1,width:x2-x1,height:y2-y1,fill:'none',stroke:c,'stroke-width':3,'vector-effect':'non-scaling-stroke'}}));const label=(glass?'glass':'ceramic')+' '+(p.relative_score*100).toFixed(1)+'%';const tx=Math.max(4,x1),ty=Math.max(18,y1-7);const bg=el('rect',{{x:tx-2,y:ty-15,width:Math.max(74,label.length*6.4),height:18,rx:3,fill:'#071019','fill-opacity':.88,stroke:c,'stroke-width':1,'vector-effect':'non-scaling-stroke'}});const t=el('text',{{x:tx,y:ty,fill:c,'font-size':12,'font-weight':800,'font-family':'ui-sans-serif,system-ui'}});t.textContent=label;overlay.appendChild(bg);overlay.appendChild(t);}}note.textContent=labels[key]+' · '+DATA.methods[key].filter(x=>x.correct).length+'/'+DATA.methods[key].length+' correct on the adaptive development source. Scores are relative and uncalibrated.';document.querySelectorAll('.method-card').forEach(b=>b.classList.toggle('selected',b.dataset.method===key));}}
...
render('siglip2_text_prompt_prototype');
    ...
    print(json.dumps({"site": str(SITE), "index_bytes": (SITE / "index.html").stat().st_size, "webp_bytes": (ASSETS / "POS_2952166.webp").stat().st_size, "offline_bytes": (SITE / "offline.html").stat().st_size}, indent=2))
